...
function errorSvg(message: string, mode: 'light' | 'dark'): string {
  ...
  <rect width="495" height="195" rx="8" fill="${bg}" stroke="${border}" stroke-width="1"/>
  <text x="247" y="101" font-family="system-ui,sans-serif" font-size="13" fill="${text}" text-anchor="middle">${message}</text>
</svg>`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const svg = renderStatsCard(result, mode);
