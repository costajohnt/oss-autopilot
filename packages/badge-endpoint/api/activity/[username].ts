...
function errorSvg(message: string, mode: 'light' | 'dark'): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="495" height="140" viewBox="0 0 495 140" fill="none">
  <rect width="495" height="140" rx="8" fill="${bg}" stroke="${border}" stroke-width="1"/>
  <text x="247" y="75" font-family="system-ui,sans-serif" font-size="13" fill="${text}" text-anchor="middle">${message}</text>
</svg>`;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const svg = renderActivityGraph(result, mode);
