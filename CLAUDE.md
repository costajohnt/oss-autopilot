...
├── src/                      # TypeScript CLI source (dev only)
└── dist/
    └── cli.bundle.cjs        # Pre-bundled CLI (zero deps, committed to git)
...
After making changes to `src/`, always run `npm run bundle` to regenerate `dist/cli.bundle.cjs` before committing.
