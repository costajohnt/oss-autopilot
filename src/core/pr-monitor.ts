...
export class PRMonitor {
  ...
  async fetchUserOpenPRs(): Promise<FetchPRsResult> {
    ...
    console.error(`Found ${data.total_count} open PRs`);
  ...
  private async fetchPRDetails(prUrl: string): Promise<FetchedPR | null> {
    ...
    // Fetch PR data, comments, reviews, and CI status in parallel
    const [prResponse, commentsResponse, reviewsResponse, ciStatus] = await Promise.all([
      ...
      this.getCIStatus(owner, repo, ''), // SHA will be filled after we get PR data
    ]);
    ...
    // Get actual CI status with the correct SHA
    const actualCIStatus = await this.getCIStatus(owner, repo, ghPR.head.sha);
  ...
  private async getCIStatus(owner: string, repo: string, sha: string): Promise<CIStatus> {
    ...
    try {
      ...
      if (statusCode === 401) {
        console.error(`[AUTH ERROR] CI check failed: Invalid token`);
      } else if (statusCode === 403) {
        console.error(`[RATE LIMIT] CI check failed: Rate limit exceeded`);
      }
      return 'unknown';
    }
  }
