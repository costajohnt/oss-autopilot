...
3. **Diagnose and fix**, by item type:
   - *Implement mode* (the prompt says `OVERNIGHT IMPLEMENT MODE` and names an issue, not a PR): there is no PR head. Worktree from the upstream default branch on `overnight/issue-<number>-<yyyy-mm-dd>` at `~/.oss-autopilot/worktrees/<owner>-<repo>-issue-<number>`; the fork remote is the user's fork of the repo (`gh repo view <login>/<repo> --json parent` confirms it), cloned or added the same way. Read the issue and every comment (`gh issue view <url> --comments`): the discussion often names the fix, the pitfall, or a maintainer's constraint, and the curated list's own note for this issue (if the list is readable) is the user's vet. Write a failing test that reproduces the issue first, then the smallest change that makes it pass. If the issue needs a design decision the thread does not settle, or the repo's contributing rules require claiming the issue first, stop and report `blocked` with that in `NOTE`. Never comment on the issue.
