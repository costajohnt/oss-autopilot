...
1. **Fetch PR Comments**
   ...
   Focus on the most recent comments from maintainers (not the PR author)

2. **Identify Key Points**
   For each maintainer comment, identify:
   - What they're asking for (code changes, clarification, tests, etc.)
   - The specific files/lines they're referencing
...
- "Post this response" - Post via `gh pr comment`
- "Edit first" - Let user modify
- "Skip" - Don't post

**Before Posting:**
Always confirm with user via AskUserQuestion. Use `gh pr comment` or equivalent MCP tool to post.
